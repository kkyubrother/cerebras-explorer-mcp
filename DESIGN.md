...
두 개 이상의 current source path를 묶는 comparison claim은 해당 claim과 bounded observation으로 focused corroboration을 한 번 더 수행한다. 이 focused pass는 temperature 0/top-p 1의 deterministic sampling을 사용한다. Packet에는 claim-local evidence와 관찰된 모든 exact current implementation/config source를 넣으며, parent hint나 sibling claim을 source 제외 근거로 사용하지 않는다. Audited request slice와 해당 sub-goal의 question/proof condition/constraints만 omission boundary로 사용하고, 같은 directory나 auth vocabulary만 공유하는 source는 누락 근거로 삼지 않는다. Supporting evidence는 결과와 무관하게 claim이 직접 인용한 ref의 subset으로 강제하며, 두 verifier가 primary verifier가 지지한 모든 source path와 전체 관계에 동의할 때만 support를 유지한다. Primary 또는 focused verifier가 이미 두 source path 이상을 가진 comparison을 semantic/category mismatch로 거절하면 Explorer call 전체에서 한 번만 같은 evidence의 deterministic claim correction을 허용한다. 교정 claim은 claim-local primary verifier와 all-exact-source focused verifier가 모두 모든 cited path를 승인해야만 transactionally 기존 claim을 대체한다. Invalid/provider failure나 재검증 불일치는 원래 gap을 보존하며 repository action을 추가하지 않는다. Claim prose의 citation-only internal observation ID는 semantic verification 전에 제거하고, 남은 unobserved ID는 invalid candidate로 격리해 parent handoff에 전달하지 않는다. Canonical frontend access synthesis도 deterministic sampling을 사용하고 관찰된 redirect literal을 그대로 쓰도록 한다. Redirect shape guard는 `to /ai`와 `to the /ai page`처럼 명시적 route가 있는 자연스러운 표현을 모두 허용하되 route가 없는 redirect 주장은 계속 거절한다. 이 내부 확인은 parent schema에 진단 필드를 추가하지 않는다.
