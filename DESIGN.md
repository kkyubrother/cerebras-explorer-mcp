...
- `budget_exhausted` retry recipes preserve the sanitized scope hard boundary
  while asking the parent agent to narrow the task, file, symbol, or text anchor.
...
spec 018에서 transcript은 운영 디버깅 채널로 재정의되었다. `CEREBRAS_EXPLORER_LOG_PATH`가 설정되면 `explore_repo`, 6개 wrapper, `explore`가 호출별 JSONL 파일을 만들고, 파일명과 모든 record에는 같은 UUID `callId`가 들어간다. 각 record는 `{ t, type, callId, ...data }` 형태이며 기본적으로 response redaction과 같은 정책을 통과한다. tool record에는 compact `args`/`result` summary가 들어가지만 raw file content와 complete tool result JSON은 기본적으로 남기지 않는다. `CEREBRAS_EXPLORER_LOG_RAW=true`일 때 planning control record를 제외한 일반 transcript record를 raw로 보존하고 final meta에는 `redacted` 상태를 남긴다. 잘못 제안된 목표에 secret이 포함될 수 있으므로 `plan_proposed`, `goal_audit`, `plan_revised`, `goal_rejected`, `subgoal_state` planning record는 raw 모드에서도 항상 redaction을 거친다.

spec 028의 structured trust pipeline은 `claim`, `verdict`, `repair`, `safety_limit`, `final`, `usage`를 별도 운영 record로 남긴다. 이 record들은 필드 allowlist와 redaction을 `LOG_RAW=true`에서도 강제하며, 검증 전 claim/verifier/assistant prose를 복사하지 않는다. Structured-path `assistant` record도 본문 대신 길이, 허용된 tool 이름, 정규화된 finish reason만 보존한다. Repair는 시작 후 성공·실패·취소 중 하나의 terminal outcome을 남기며, usage는 완료된 호출뿐 아니라 실제로 발행됐다가 실패·취소된 provider 호출도 센다.
