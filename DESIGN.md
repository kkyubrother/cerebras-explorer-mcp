...
`map_change_impact`의 unanchored wrapper mode는 exact grep 한 번에서 역할별 후보와 match line을 골라 bounded read batch로 전환한다. 첫 batch가 test/docs만 포함해 implementation evidence가 전혀 없을 때만 같은 predicate를 implementation scope에 한 번 더 적용하고, repair는 한 repository action으로 제한한다.
