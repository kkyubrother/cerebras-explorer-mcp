...
mcpServers:
  - name: cerebras-explorer
    ...
    args:
      - "-y"
      - "github:kkyubrother/cerebras-explorer-mcp#v0.6.1"
    env:
...
Continue를 재시작 후 Agent 모드로 전환하고, 기본 도구 목록(`explore_repo`, `find_relevant_code`, `trace_symbol`, `map_change_impact`, `explain_code_path`, `collect_evidence`, `review_change_context`, `explore`)이 보이는지 확인합니다.
