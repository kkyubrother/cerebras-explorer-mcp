...
>
> **상태** (2026-06-14 갱신): #1~#5는 모두 spec으로 소진 완료 — #1·#2 → specs/013 (+#2 언어 확장은 specs/015), #3 → specs/014, #4 → specs/012, **#5 → specs/026 (PR #43/#44 머지 완료)**. 남은 씨앗: (a) adoption 케이스 입력-기대 키워드 에코 정리 (spec 025 Out of Scope에서 이월) → **specs/027-adoption-echo-cleanup 진행** (2026-06-14 분리). 함께 이월됐던 **parent-agent 실측 A/B 자동화는 2026-06-14 검토로 드롭** — 사유: npm test 밖에서 외부 parent CLI를 spawn하고 transcript를 per-parent adapter로 파싱하는 standing liability라, parent CLI 포맷 드리프트 시 record-only 리포트가 조용히 틀리고(never-gating이라 CI가 못 잡음) brittleness를 달래려다 zero-dep invariant를 압박한다. 측정 가치는 TESTING.md "수동 관찰 절차" §1–§2가 정직한 stand-in으로 충분히 커버하므로 도구화 비용을 정당화하지 못한다. 재론 조건: 동일 환경의 두 parent에서 explorer 유무 토큰 차이를 수동 절차로 반복 관측하기 어렵다는 실측 수요가 생길 때 — 그때도 정책 적합 형태는 우선 수동 절차 보강이다. `.npmignore`/`.dockerignore` 옵트인 처리는 2026-06-14 검토로 백로그에서 제거 — spec 014의 `.cerebras-explorer.json:extraIgnorePatterns`로 충분히 대체 가능하고 별도 자동/옵트인 표면을 늘릴 필요가 없다. `src/benchmark/evaluator.mjs`의 vacuous `stopped_by_budget_equals` check도 2026-06-14 삭제했다. (구 entrypoint 카테고리 씨앗 — Lambda/K8s CronJob/Pub-Sub — 은 2026-06-12 검토로 제거, 사유는 #2 절 참조.) 새 후보가 생기면 본 문서에 절을 추가하고, 진행할 때 개별 `specs/NNN-slug/` 디렉토리를 끊은 뒤 해당 절을 "→ specs/NNN-slug 완료"처럼 갱신한다.
...
→ `specs/026-trace-symbol-cross-check/` 로 완료 (PR #43 US1/US2 + PR #44 US3/Polish, 2026-06-12 머지). deterministic usage cross-check gate + `usage_cross_check_missing` warning + symbolContext 결정적·다양성 보존 절단 + symbol-first 프롬프트 cross-check 지시로 구현됨. SC-001 라이브 위반 0건, 기존 trace-symbol 벤치 비회귀 1.0.
