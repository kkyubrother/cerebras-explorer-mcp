...
## Phase 1. Fast-path stabilization

목표: 큰 schema 변경 없이도 현재의 과도한 low confidence를 먼저 완화한다.

작업:

- `repo_git_blame` 결과의 line 범위를 `observedRanges`에 기록
- `repo_git_diff` / `repo_git_show` 결과에서 diff hunk를 파싱해 changed line range를 `observedRanges`에 기록
- `repo_git_log`만 사용된 경우에는 evidence drop을 곧바로 hallucination으로 간주하지 않도록 confidence 로직을 완화
- git 도구를 사용했고 `recentActivity` 또는 구조화된 git result가 존재하면 confidence factor에 별도 힌트를 추가
