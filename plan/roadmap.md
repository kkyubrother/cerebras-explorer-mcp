...
| **P1** | 1.2 | 심볼 인덱스 (regex 기반 경량 구현) | ★★★★★ | Large | ✅ DONE |
...
| **P3** | 4.3 | 내부 프로바이더 추상화 메모 | ★★★ | Medium | 참고 |
...
- [x] 특화 MCP 도구 추가 (`explain_symbol`, `trace_dependency`, `summarize_changes`, `find_similar_code`) — `src/mcp/server.mjs`
- [~] 출력 스키마 문서화 — 구현은 완료됐고, 공개 문서 정리는 후속 과제
...
**5.1 인터랙티브 세션 모드:**
- `SessionStore`: TTL(기본 30분)/maxCalls(기본 5회) 기반 인메모리 세션 관리 (`isExhausted()` API는 구현됐으나 현재 `explore()`에서 호출하지 않아 maxCalls 초과 시에도 호출이 거부되지 않음)
...
- 지원 필드: `defaultBudget`, `defaultScope`, `extraIgnoreDirs`, `projectContext`, `keyFiles`, `entryPoints` (단, `entryPoints`는 정규화만 하고 런타임에서 소비하지 않음 — future-use)
...
### Milestone 4: Ecosystem (P3) — ✅ 핵심 구현 완료, 후속 개선 항목 잔존 (2026-04-08)
> **목표:** 유연성과 확장성 확보
> 문서화된 제품 목표는 Cerebras 기반 explorer이며, 이 섹션의 provider abstraction 관련 내용은 공개 계약이 아니라 내부 구현 메모로 본다.

- [x] 내부 프로바이더 추상화 구현 존재 — `src/explorer/providers/` (공개 목표 아님)
...
- [x] 매크로 도구 (`repo_symbol_context`, `repo_grep` contextLines) — `src/explorer/repo-tools.mjs` (Phase 2에서 구현)
...
**4.3 내부 프로바이더 추상화 메모:**
- `AbstractChatClient`: chat client 인터페이스 계약
- provider abstraction 및 failover 관련 코드가 `src/explorer/providers/` 아래에 존재
- 다만 이 기능은 현재 공개 문서에서 지원 계약으로 약속하지 않으며, 제품 목표는 Cerebras 기반 explorer 유지

**테스트 추가:** 23종 (providers.test.mjs) + mcp-server 도구 목록 검증

**잔여 개선 항목:**
- `find_similar_code`의 구조화된 `similarity` 수치는 아직 미구현
- `repo_symbol_context.depth > 1` 호출 체인 확장은 아직 미구현
- 설정 파일의 `languages`, `customSymbolPatterns`는 future-use 수준으로만 남아 있음

**현재 상태:** 다양한 환경/프로바이더에서 유연하게 동작하는 탐색 플랫폼이며, 일부 정밀도 향상 항목만 후속 과제로 남아 있음.
...
단순 텍스트 검색을 넘어 regex 기반 경량 심볼 추적 + git 이력 = "코드의 의미와 역사를 아는 탐색기".
...
- **zero dependencies 원칙 유지**: 현재 심볼 인덱싱은 regex 기반 경량 구현
