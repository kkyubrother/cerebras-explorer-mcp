...
- 본 작업은 `package.json`의 현재 버전과 충돌하지 않는 **별도 변경 묶음**을 생성한다. 버전이 `0.2.0`에 머문 상태라면 기존 `## v0.2.0 - 2026-05-19` 아래에 날짜가 붙은 후속 섹션을 두고, 향후 버전 승격 시점의 릴리즈 헤딩은 별도 release 작업에서 확정한다. 다섯 항목은 `### Added`/`### Changed`/`### Documentation` 또는 동등한 항목 라벨로 분배되, 외부 독자가 “이 묶음은 본 플랜의 산출물”임을 알 수 있도록 묶음 머리말 한 줄을 둔다.
