...
Evidence ids are opaque exact tokens, so `E5` does not authorize `E5:search`. An out-of-claim ref receives one bounded correction. If the final response is otherwise schema-valid but repeats that violation, runtime quarantines only the affected verdict as `insufficient` with no supporting refs. Unknown, duplicate, or missing claim ids and malformed control remain fatal; quarantine can never promote a claim.
