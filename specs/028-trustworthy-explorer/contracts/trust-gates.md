...
- `reasonCode=uncovered_request` is valid only with an `insufficient` verdict. A structured verifier-reported uncovered facet deterministically normalizes any result on the same canonical verdict to `insufficient/uncovered_request`; it cannot create a sibling goal or late goal-audit batch.
