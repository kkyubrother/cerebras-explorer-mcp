...
| The semantic verifier is another model and can be wrong | It sees only bounded claims and reconstructed evidence, uses deterministic settings, cannot promote unreferenced evidence, and is backed by known-answer evaluation. Multi-path comparisons and certificate-only refutations receive focused independent corroboration; disagreement is non-complete. |
...
| Extra model passes increase latency | Cost/latency are observable, not acceptance gates; allow one initial plan/audit, at most one corrected plan/audit, one batched semantic verification plus at most one focused corroboration per supported comparison spanning two or more current source paths or certificate-only refutation, and at most one repair/reverification cycle. Each verifier pass may submit at most one bounded late-goal audit batch, never a new planner or repair loop. |
