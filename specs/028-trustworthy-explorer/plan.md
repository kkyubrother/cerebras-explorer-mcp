...
**Scale/Scope**: Existing structured exploration path across JavaScript/TypeScript, Python, Go, and mixed large repositories; at least seven real repositories in the trust suite; production changes concentrated in runtime, prompt, critic/coverage, schemas, MCP server, transcript, benchmark, tests, docs, and integration manifests
...
4. Permit one corrected planner pass only for missing request parts or goals requiring decomposition. Re-audit once. Reject/log planner inventions without blocking; retain user-required blockers as terminal required gaps. Materialize every remaining uncovered or undecomposable requested part as a blocked `planning_incomplete` required goal so it participates in state reduction rather than remaining a free-floating diagnostic.
...
| Extra model passes increase latency | Cost/latency are observable, not acceptance gates; allow one initial plan/audit, at most one corrected plan/audit, one batched semantic verification plus at most one focused corroboration per supported comparison spanning three or more current source paths, and at most one repair/reverification cycle. Each verifier pass may submit at most one bounded late-goal audit batch, never a new planner or repair loop. |
