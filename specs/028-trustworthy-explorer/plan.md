...
| The semantic verifier is another model and can be wrong | It sees only bounded claims and reconstructed evidence, uses deterministic settings for focused checks, cannot promote unreferenced evidence, and is backed by known-answer evaluation. Multi-path focused packets retain every exact current implementation/config source so hints or sibling claims cannot hide an omission; one same-evidence correction may replace a disputed claim only after claim-local primary and all-source focused verification both agree. Certificate-only refutations also receive focused independent corroboration; unresolved disagreement is non-complete. |
...
| Extra model passes increase latency | Cost/latency are observable, not acceptance gates; allow one initial plan/audit, at most one corrected plan/audit, one batched semantic verification plus one focused corroboration for each supported comparison spanning two or more current source paths or certificate-only refutation, at most one Explorer-wide same-evidence comparison correction with claim-local primary and focused re-verification, and at most one repository evidence-repair/reverification cycle. Each verifier pass may submit at most one bounded late-goal audit batch, never a new planner or repair loop. |
