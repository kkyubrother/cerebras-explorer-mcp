...
- **FR-029**: A goal that is valid but too broad MUST be decomposed, and an explicit requested part omitted by the initial plan MUST be inserted as a required goal, through at most one bounded plan revision.
- **FR-030**: The explorer MUST NOT recursively re-plan. After one plan revision, every remaining uncovered or undecomposable requested part MUST be materialized as a blocked required goal and honest gap rather than remain a diagnostic or trigger another planning loop.
...
- **FR-033**: The parent-facing gap MUST state only the requested part and the action-relevant blocker. It MUST NOT expose rejected internal goals, planning history, feasibility scores, or repeated diagnostic explanations.
