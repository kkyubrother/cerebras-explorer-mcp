...
export function buildExplorerSystemPrompt({ repoRoot, budgetConfig, language, projectContext, previousSummaries, keyFiles }) {
  const parts = [
    ...
    '## FINAL OUTPUT CONTRACT',
    '{',
    '  "directAnswer": "string|null — optional short answer; use null to let runtime fill from answer",',
    '  "answer": "string — direct answer to the task",',
    '  "summary": "string|null — short synthesis of key findings",',
    ...
    '  "followups": [{"description": "...", "priority": "recommended|optional", "query": "optional follow-up query or null"}]',
    '}',
    '- Strict structured output requires all declared fields; use null for optional scalar/object values that are not applicable.',
...
export function buildFinalizePrompt() {
  return [
    ...
    'SCHEMA REQUIREMENTS:',
    '  • Required fields: all schema-declared fields; use null for optional scalar/object values that are not applicable',
    '  • Optional collections may be [] when empty; optional scalar/object fields should be null rather than omitted',
    '  • evidence items include path, startLine, endLine, why; use null for optional metadata such as evidenceType when not applicable',
    '  • followups items: { description, priority (recommended|optional), query } with query null when not needed',
