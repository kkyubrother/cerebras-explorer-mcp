...
const CANONICAL_DECOMPOSITION_PATTERNS = Object.freeze([
  ...
  '- For wrapper:find_relevant_code, a request shaped as "I need to update X. What exact targets should I read?" uses update only as location relevance context. Keep the fixed locations, relevance, and smallest_set leaves; do not invent an outdated comparison, edit execution, or impact goal unless the caller explicitly requests it.',
  '- For wrapper:find_relevant_code:smallest_set, define a bounded useful target set whose cited files directly cover the requested location and companion verification surface. Never define it as the only, must, or exhaustive set unless the caller explicitly asks for exhaustive proof.',
...
const CLAIM_SYNTHESIS_SYSTEM_PROMPT = [
  ...
  '- For wrapper:find_relevant_code:relevance, when the request asks which code controls a public or structured output contract, cite the formatter/schema definition and one exact current production caller or adapter that connects it to the returned output. Definitions alone do not establish the production boundary.',
  ...
  '- For wrapper:find_relevant_code:smallest_set, describe cited implementation and companion verification paths as a bounded useful target set. Never call them the only, must, or exhaustive set unless that stronger boundary was explicitly requested and proved.',
...
const SEMANTIC_VERIFIER_SYSTEM_PROMPT = [
  ...
  '- For wrapper:find_relevant_code:relevance, a claim about code controlling a public or structured output is supported only when its cited packet includes the formatter/schema definition and an exact current production caller or adapter connecting it to the returned output.',
  ...
  '- For wrapper:find_relevant_code:smallest_set, support a claim that presents exact cited implementation and companion verification paths as a bounded useful target set. Treat only, must, or exhaustive language as overgeneralized unless the original request requires and the evidence proves that stronger boundary.',
