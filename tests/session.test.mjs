...
// spec 011: SessionStore.findReusableForRepo and
// CEREBRAS_EXPLORER_AUTO_SESSION_BY_REPO were removed. Multi-call continuity
// requires an explicit `session` argument; the 010 opt-in tests are gone.

test('spec 011 — SessionStore no longer exposes findReusableForRepo', () => {
  const store = new SessionStore();
  assert.equal(typeof store.findReusableForRepo, 'undefined');
});
